...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
  ...
  .grammar-link { grid-column: 1 / -1; width: 100%; display: flex; align-items: center; gap: 14px; color: var(--ink); text-decoration: none; background: linear-gradient(135deg, #f0efff, #f8ffe3); border-color: #756bdc; padding: 17px 18px; position: relative; overflow: hidden; }
  .grammar-link::after { content: "G"; position: absolute; right: 45px; top: -22px; color: rgba(98, 88, 217, .08); font-size: 92px; font-weight: 900; transform: rotate(-8deg); pointer-events: none; }
  .grammar-link .ico { width: 46px; height: 46px; flex: none; display: flex; align-items: center; justify-content: center; margin: 0; border-radius: 12px; background: #6258d9; color: #fff; box-shadow: 3px 3px 0 #dfff74; font-size: 23px; }
  .grammar-link .copy { min-width: 0; flex: 1; position: relative; z-index: 1; display: grid; gap: 4px; }
  .grammar-link .nm { color: #433ca8; }
  .grammar-link .go { position: relative; z-index: 1; color: #433ca8; font-size: 23px; font-weight: 900; }
  .grammar-link:focus-visible { outline: 3px solid var(--amber); outline-offset: 3px; }
...
async function openGrammarQuest() {
  if (!S.login) return;
  const button = document.getElementById("mnGrammar");
  if (button) button.disabled = true;
  try {
    const r = await api("/api/grammar/session", S.login);
    if (!r.ok || !r.token) throw new Error(r.error || "문법 퀘스트를 열지 못했어요.");
    const params = new URLSearchParams({ ticket: r.token, section: S.login.section, id: S.login.id });
    window.location.href = "https://grammar-quest-kids.elated-kiwi-8872.chatgpt.site/#" + params.toString();
  } catch (e) {
    toast(e && e.message ? e.message : "문법 퀘스트를 열지 못했어요.");
...
function viewHome() {
  ...
  render(\`
    ...
    <div class="menu-grid">
      <button class="menu-btn grammar-link" id="mnGrammar" aria-label="문법 퀘스트를 열고 오늘의 코인 받기">
        <span class="ico">🧩</span>
        <span class="copy"><span class="nm">문법 퀘스트</span><span class="st">5문제 완주하고 하루 한 번 🪙 50코인</span></span>
  ...
  document.getElementById("mnGrammar").onclick = openGrammarQuest;
...
const SECTIONS = {
  g1: {
    name: "1학년",
    // maxLen: 이 레벨에서 허용할 최대 철자 수(안전장치). avoidDays: 겹치지 않게 피할 최근 며칠치.
    // ※ 저학년은 쓸 수 있는 쉬운 단어 자체가 적어서, 피할 기간을 길게 잡으면
    //   "새 단어가 없다" → 재생성 → 수준을 벗어난 어려운 단어가 나온다.
    maxLen: 7, avoidDays: 5,
...
// AI가 레벨 기준을 어겼을 때 걸러 주는 안전장치.
// 철자 수는 완벽한 잣대는 아니지만, 저학년 화면에 abandonment·hypothesis 같은
// 수능 단어가 통째로 뜨는 일은 이것만으로도 거의 막힌다. (maxLen 이 없는 레벨은 통과)
function fitsLevel(word, lv) {
  if (!lv || !lv.maxLen) return true;
  const w = String(word || "").trim();
  if (!w) return false;
  // "ice cream" 처럼 두 낱말이면 가장 긴 낱말 길이로 본다.
  return w.split(/[\s-]+/).reduce((m, p) => Math.max(m, p.length), 0) <= lv.maxLen;
}
...
const GRAMMAR_REWARD = 50;          // '문법 퀘스트' 5문제 완주 보상(하루 1번)
const GRAMMAR_ORIGIN = "https://grammar-quest-kids.elated-kiwi-8872.chatgpt.site";
const GRAMMAR_TICKET_TTL_MS = 2 * 60 * 60 * 1000;
...
// 학년(레벨)별로 지금까지 쌓인 단어 통계 — /stats 페이지에서 쓴다.
// 중복이 허용되므로 '총 개수'와 '서로 다른 단어 수'는 다를 수 있다.
async function wordStats(db) {
  const rows = [];
  for (const key of Object.keys(SECTIONS)) {
    const sets = await recentDaySets(db, key, 400);   // 400일이면 1년 넘게 커버
    const uniq = new Set();
    let total = 0;
    for (const s of sets) for (const w of (s.words || [])) {
      total++;
      uniq.add(String(w && w.word || "").trim().toLowerCase());
    }
    const days = sets.map(s => s.day).sort();
    rows.push({
      key, name: SECTIONS[key].name,
      days: sets.length, total, unique: uniq.size,
...
function statsHtml(rows) {
  ...
  const tr = rows.map(r =>
    `<tr><td>${esc(r.name)}</td><td class="n">${r.days}</td><td class="n">${r.total}</td>` +
    `<td class="n">${r.unique}</td><td>${esc(r.first)} ~ ${esc(r.last)}</td></tr>`).join("");
  return `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8">` +
    `<meta name="viewport" content="width=device-width,initial-scale=1"><title>${APP_NAME} · 단어 통계</title>` +
    `<style>body{font-family:"Nanum Myeongjo",serif;background:#f3efe3;color:#2b2b2b;padding:20px;max-width:720px;margin:0 auto}` +
    `h1{font-size:20px;color:#1f4e8c}table{border-collapse:collapse;width:100%;background:#fffef7;box-shadow:0 2px 0 rgba(0,0,0,.12)}` +
    `th,td{border:1px solid #ddd6c4;padding:8px 10px;font-size:14px;text-align:left}th{background:#eae4d3;color:#1f4e8c}` +
    `td.n,th.n{text-align:right}p{font-size:13px;color:#666}</style></head><body>` +
    `<h1>${APP_NAME} — 학년별 단어 통계</h1>` +
    `<table><tr><th>단계</th><th class="n">만든 날짜 수</th><th class="n">총 단어 수</th><th class="n">서로 다른 단어</th><th>기간</th></tr>` +
    tr +
    `<tr><th>합계</th><th class="n">${sum("days")}</th><th class="n">${sum("total")}</th><th class="n">-</th><th>-</th></tr></table>` +
    `<p>하루에 ${WORDS_PER_DAY}개씩 만들어집니다. 같은 단어가 다시 나올 수 있어서(복습) ` +
    `'총 단어 수'보다 '서로 다른 단어'가 적을 수 있어요.</p></body></html>`;
}
...
async function generateWords(env, section, avoidWords, cf) {
  ...
  const prompt =
    ...
    `- 명사·동사·형용사를 골고루 섞되, 난이도는 위 '난이도 기준'을 절대 벗어나지 않는다.\n` +
    (lv.maxLen ? `- 철자 ${lv.maxLen}자를 넘는 긴 단어는 쓰지 않는다. 시험용 고급 어휘·추상적 개념어는 절대 넣지 않는다.\n` : "") +
    (avoid ? `- 되도록 다음 단어와 겹치지 않게 해 주세요(단, 겹침을 피하려고 수준을 올리지는 말 것 — 난이도 기준이 우선입니다): ${avoid}\n` : "");
  ...
  // 먼저 최근 단어와 겹치지 않는 것만 고르고, 부족하면 새 단어를 한 번 더 생성해 채운다.
  // 그래도 모자라면 최근에 나왔던 단어를 그대로 다시 쓴다(아래 '중복 허용' 부분).
  const rawPool = rawWords.slice();   // 중복이라 걸러낸 단어까지 포함한 원본 — 마지막에 채울 때 쓴다
  const fresh = rawWords.filter(w => {
    const key = String(w && w.word || "").trim().toLowerCase();
    return key && !avoidSet.has(key) && fitsLevel(key, lv);
  });
  ...
  if (words.length < WORDS_PER_DAY - 4) {
    ...
    // ※ 재요청에도 반드시 '대상·난이도 기준'을 그대로 다시 넣는다.
    //   (예전엔 이 프롬프트에 수준 정보가 빠져 있어서, 재요청이 걸리면
    //    1학년 반에 abandonment·hypothesis 같은 수능 단어가 통째로 들어갔다.)
    const retryPrompt =
      `대상: ${who} (한국 학습자가 배우는 영어 어휘).\n난이도 기준: ${lv.desc}\n\n` +
      `앞서 생성한 단어 중 이미 사용된 단어가 많았습니다.\n` +
      `위 난이도 기준에 맞는 새로운 영단어 ${WORDS_PER_DAY - words.length + 8}개를 만들어 주세요.\n` +
      `- 난이도 기준을 벗어난 어려운 단어는 절대 넣지 마세요. 겹치지 않게 하려고 수준을 올리는 것은 금지입니다.\n` +
      (lv.maxLen ? `- 철자 ${lv.maxLen}자를 넘는 단어는 쓰지 마세요.\n` : "") +
      `- meaning 은 ${who}이(가) 바로 이해할 한글 뜻, example 은 그 단어가 원형 그대로 들어간 쉬운 영어 한 문장.\n` +
      `- 되도록 다음 단어는 다시 쓰지 마세요: ${extraAvoid.join(", ")}\n` +
    ...
    try {
      const retryRes = await geminiFetch(env, endpoint, {
        system_instruction: { parts: [{ text: "당신은 한국 학습자를 위한 영어 어휘 교육 전문가입니다. 요청받은 대상과 수준에 딱 맞는 영단어만 정확한 JSON 형식으로 만듭니다." }] },
      ...
      if (retryRes.ok && retryBody && retryBody.candidates && retryBody.candidates[0]) {
        ...
        try {
          ...
          const retryRaw = Array.isArray(retryParsed.words) ? retryParsed.words : [];
          for (const w of retryRaw) rawPool.push(w);
          const retryFresh = retryRaw.filter(w => {
            const key = String(w && w.word || "").trim().toLowerCase();
            return key && !avoidSet.has(key) && !usedNow.includes(key) && fitsLevel(key, lv);
          });
  ...
  // ── 중복 허용: 새 단어가 바닥나도 실패시키지 않는다 ──
  // 저학년은 쓸 수 있는 쉬운 단어 자체가 몇백 개뿐이라 며칠만 지나면 '겹치지 않는 새 단어'가
  // 동나고, 예전에는 그때 "새 단어를 충분히 만들지 못했어요" 로 아예 생성이 실패했다.
  // 같은 단어가 다시 나오는 것은 복습이라 문제가 없으므로, 부족분은 최근에 나왔던 단어로 채운다.
  // (수준 필터 fitsLevel 은 여기서도 그대로 지킨다 — 겹침을 피하려고 어려운 단어를 넣지는 않는다.)
  if (words.length < WORDS_PER_DAY) {
    const repeats = rawPool.filter(w => fitsLevel(String(w && w.word || "").trim(), lv));
    words = sanitizeWords(words.concat(repeats), WORDS_PER_DAY);
  }

  // 여기까지 와서도 모자라면 중복 때문이 아니라 AI 응답 자체가 부실하거나(개수 부족)
  // 수준에 맞는 단어가 거의 없는 경우다. 최소 하루 목표(10개)는 넘겨야 내보낸다.
  const MIN_WORDS = Math.min(DAILY_GOALS[0], WORDS_PER_DAY - 4);
  if (words.length < MIN_WORDS) {
    const levelOut = rawPool.filter(w => !fitsLevel(String(w && w.word || "").trim(), lv)).length;
    return { error: `AI가 단어를 충분히 만들지 못했어요(${words.length}개 확보 / AI 응답 ${rawPool.length}개 중 수준에 안 맞는 단어 ${levelOut}개). 다시 시도해 주세요.` };
  }
  return { words };
}
...
async function ensureTodaySet(env, db, section, day, cf) {
  ...
  // 최근 며칠치 단어와 겹치지 않게. (저학년은 쓸 수 있는 쉬운 단어가 적어서 짧게 잡는다 — SECTIONS.avoidDays)
  const recent = await recentDaySets(db, section, (SECTIONS[section] && SECTIONS[section].avoidDays) || 15);
  const avoid = [];
...
// ── API 라우팅 ──
async function handleApi(env, db, path, d, cf, origin) {
  // ── 앱 기본 정보 (시작 화면용) ──
  if (path === "/api/info") {
  ...
  if (path === "/api/student/save") {
    ...
    d.data.grammarTicket = cur.grammarTicket || null; // 문법 완료권도 서버가 발급하고 관리한다
  ...
  // ── 문법 퀘스트: 1회용 완료권 발급 ──
  if (path === "/api/grammar/session") {
    if (!validSection(d.section)) return fail("반 정보가 없어요.");
    const id = validId(d.id);
    if (!id) return fail("기기 정보가 없어요.");
    const data = await getStudent(db, d.section, id);
    if (!data) return fail("등록되지 않은 학생이에요. 처음 화면에서 다시 시작해 주세요.");
    data.rewardHistory = data.rewardHistory || {};
    const token = crypto.randomUUID();
    data.grammarTicket = { token, expiresAt: Date.now() + GRAMMAR_TICKET_TTL_MS };
    await putStudent(db, d.section, id, data);
    return ok({ token, rewardedToday: !!data.rewardHistory["grammar_" + kst().date] });
  }

  // ── 문법 퀘스트: 5문제 완주 확인 및 코인 지급 ──
  if (path === "/api/grammar/complete") {
    if (origin !== GRAMMAR_ORIGIN) return fail("허용되지 않은 문법 퀘스트 요청이에요.", 403);
    if (!validSection(d.section)) return fail("반 정보가 없어요.");
    const id = validId(d.id);
    const token = String(d.ticket || "").trim();
    if (!id || !token) return fail("완료권 정보가 없어요.");
    const data = await getStudent(db, d.section, id);
    if (!data) return fail("등록되지 않은 학생이에요.");
    const ticket = data.grammarTicket;
    if (!ticket || ticket.token !== token) return fail("이미 사용했거나 올바르지 않은 완료권이에요.", 403);
    data.grammarTicket = null;
    if (!Number.isFinite(ticket.expiresAt) || ticket.expiresAt < Date.now()) {
      await putStudent(db, d.section, id, data);
      return fail("완료권 시간이 지났어요. 매일 영단어에서 다시 시작해 주세요.", 403);
    }
    data.rewardHistory = data.rewardHistory || {};
    const rewardKey = "grammar_" + kst().date;
    if (data.rewardHistory[rewardKey]) {
      await putStudent(db, d.section, id, data);
      return ok({ already: true, reward: 0, coins: Number(data.coins || 0) });
    }
    data.rewardHistory[rewardKey] = true;
    data.coins = Number(data.coins || 0) + GRAMMAR_REWARD;
    await putStudent(db, d.section, id, data);
    return ok({ already: false, reward: GRAMMAR_REWARD, coins: data.coins });
  }
...
function grammarJsonResponse(body, status) {
  return new Response(JSON.stringify(body), {
    status,
    headers: {
      "Content-Type": "application/json; charset=utf-8",
      "Cache-Control": "no-store",
      "Access-Control-Allow-Origin": GRAMMAR_ORIGIN,
      "Vary": "Origin",
    },
  });
}
...
export default {
  async fetch(request, env) {
    ...
    if (method === "OPTIONS" && path === "/api/grammar/complete") {
      if (request.headers.get("Origin") !== GRAMMAR_ORIGIN) return new Response(null, { status: 403 });
    ...
    if (method === "GET" || method === "HEAD") {
      ...
      // 학년별로 지금까지 몇 개의 단어가 쌓였는지 보는 페이지 (선생님 확인용, 개인정보 없음)
      if (path === "/stats") {
        try {
          await ensureSchema(env.DB);
          const rows = await wordStats(env.DB);
          return new Response(statsHtml(rows), { headers: { "Content-Type": "text/html; charset=utf-8", "Cache-Control": "no-store" } });
        } catch (e) {
          return new Response("통계를 읽지 못했어요: " + (e && e.message ? e.message : e), { status: 500, headers: { "Content-Type": "text/plain; charset=utf-8" } });
        }
      }
    ...
    try {
      await ensureSchema(env.DB);
      const r = await handleApi(env, env.DB, path, d, request.cf, request.headers.get("Origin"));
      return path === "/api/grammar/complete" ? grammarJsonResponse(r.body, r.status) : jsonResponse(r.body, r.status);
    } catch (e) {
      const body = { ok: false, error: "서버 오류: " + (e && e.message ? e.message : e) };
      return path === "/api/grammar/complete" ? grammarJsonResponse(body, 500) : jsonResponse(body, 500);
    }
  },
};
