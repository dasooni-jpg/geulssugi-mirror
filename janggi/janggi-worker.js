...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  #layer-mark, #layer-pieces, #layer-cells, #layer-arrow { position:absolute; inset:0; }
  #layer-mark { z-index:2; pointer-events:none; }
  #layer-arrow { z-index:5; pointer-events:none; width:100%; height:100%; }
  ...
  /* 방금 둔 수 — 떠난 자리와 놓인 자리를 다르게 보여 줌 */
  .mk.lastfrom {
    width:calc(var(--u)*.74); height:calc(var(--u)*.74); border-radius:50%;
    border:calc(var(--u)*.055) dashed rgba(120,72,20,.85);
    background:rgba(120,72,20,.14);
  }
  .mk.lastto {
    width:calc(var(--u)*1.24); height:calc(var(--u)*1.24); border-radius:50%;
    border:calc(var(--u)*.085) solid #ff9d2e;
    box-shadow:0 0 0 calc(var(--u)*.03) rgba(60,30,0,.55),
               0 0 calc(var(--u)*.30) rgba(255,157,46,.85);
    animation:landpulse 1.6s ease-out 1;
  }
  @keyframes landpulse {
...
<div id="app">
  ...
  <section id="screen-menu" class="screen active">
    ...
    <div class="menu-grid">
      ...
      <button class="menu-btn" data-go="stats">
        <span class="mi">🏅</span><span class="mt">내 전적 · 학습</span>
        <span class="md">난이도별 승 · 무 · 패와, 컴퓨터가 배운 것</span>
      </button>
    </div>
  ...
  <section id="screen-ai-setup" class="screen">
    ...
    <div class="card" style="max-width:860px; margin:0 auto;">
      ...
      <div class="toggle-row"><input type="checkbox" id="opt-learn" checked><label for="opt-learn">둔 대국에서 배우기 — 컴퓨터가 지난 판을 기억해 초반 수를 고칩니다</label></div>
    ...
    <div id="game-layout">
      <div id="board-wrap">
        <div id="board">
          ...
          <svg id="layer-arrow" viewBox="0 0 924 1024" aria-hidden="true"></svg>
  ...
  <section id="screen-rules" class="screen">
    ...
    <div class="rule-grid">
      ...
      <div class="rule-card">
        <h3>🔁 같은 자리 되풀이</h3>
        <p>같은 자리를 여러 번 오가도 <b>벌칙은 없습니다.</b> 마음 놓고 두셔도 됩니다.</p>
        <p>같은 판 모양이 여러 번 나와도 판을 끊지 않습니다. 다만 서로 <b>한 점도 잡지 못한 채 100수</b>가 지나면, 판이 더 나아가지 않는 것으로 보고 남은 기물 점수로 가립니다.</p>
        <p>컴퓨터는 <b>뜻 없이 같은 자리를 오가지 않도록</b> 되어 있어, 질 것 같다고 판을 끄는 일은 없습니다.</p>
      </div>
      <div class="rule-card">
        <h3>🏆 이기는 방법 · 점수제</h3>
        <p><b>외통</b>이면 바로 승리입니다. 그 밖에 판가름이 안 나면 <b>남은 기물 점수</b>로 가립니다.</p>
  ...
  <section id="screen-stats" class="screen">
    ...
    <div class="card" style="max-width:620px; margin:18px auto 0;">
      <div class="section-label first">📖 컴퓨터가 배운 것</div>
      <p class="hint-line" style="margin-top:0;">
        대국이 끝날 때마다 <b>컴퓨터가 두었던 초반 서른 수</b>를 자리별 승·무·패로 적어 둡니다.
        다음에 같은 자리를 만나면 그 성적을 보고 수를 고칩니다. 전에 졌던 수는 덜 두고, 이겼던 수는 더 둡니다.
        기보에 <b>📖</b> 가 붙은 수가 장부를 참고한 수입니다.
      </p>
      <div id="book-summary"></div>
      <p class="hint-line">
        같은 자리를 <b>두 판 이상</b> 겪어야 반영되고, 최선수와 차이가 큰 수까지 억지로 두지는 않습니다.
        기록은 이 기기의 브라우저 안에만 남고 어디로도 보내지 않습니다.
      </p>
      <div style="text-align:center; margin-top:14px;"><button class="btn ghost" id="btn-reset-book">배운 것 지우기</button></div>
    </div>
  </section>
</div>
...
/* 외통이 되는 수를 하나만 찾아 바로 돌려줌 — 훈수처럼 하나면 되는 곳에 씀 */
function firstMatingMove(side, n) {
  for (const m of legalMoves(side)) {
...
const PUZZLES = [
  ...
  /* ── 5수 외통 ── */
  { n: 5, cho: 'K23 R82 P87', han: 'K84 S83' },
  { n: 5, cho: 'K24 R57 P76', han: 'K83' }
];
...
/* ============================================================================
   ...
   ============================================================================ */
/* 같은 자리를 오가도 벌칙은 없다. 아래 값은 컴퓨터가 뜻 없이 왔다 갔다 하지
   않도록 스스로 피하는 데에만 쓴다(사람에게는 아무 불이익이 없음). */
const SHUTTLE_LIMIT = 4;      /* 컴퓨터가 피하기 시작하는 왕복 횟수 */
const NOCAP_LIMIT = 100;      /* 이만큼 서로 한 점도 못 잡으면 점수로 가림 */
...
/* 이 수가 '뜻 없는 되풀이'인가 — 컴퓨터가 그런 수를 피하는 데에만 씀 */
function isPointlessRepeat(m, side, stack, posKeys) {
  if (m < 0) return false;
  const cap = mk(m);
...
let yieldNow = null;
let rootBan = null;      /* 뿌리에서 걸러낼 수를 가려내는 검사기 (뜻 없는 되풀이 등) */
let bookBonus = null;    /* 지난 대국에서 배운 점수 보정 (수 → 가감점) */
const BOOK_WINDOW = 45;  /* 최선수와 이만큼 안쪽인 수들 사이에서만 배운 것을 반영 */
async function searchBest(side, level) {
  ...
  /* 지난 대국에서 배운 것 반영 —
     최선수와 큰 차이가 없는 수들 사이에서만 손질한다. 배운 것 때문에
     기물을 그냥 내주는 일이 없도록 창을 좁게 둔다. */
  if (bookBonus && list.length > 1) {
    const top = list[0].s;
    for (const it of list) {
      if (it.s >= top - BOOK_WINDOW) it.s += bookBonus(it.m);
    }
    list.sort((a, b) => b.s - a.s);
  }
...
const S = {
  ...
  korean: false, bikjang: false, sound: true, learn: true
};
...
/* 판 좌표(0~8, 0~9)를 SVG 눈금으로 */
const svgX = c => PAD + c * CELL;
const svgY = r => PAD + r * CELL;
...
function renderMarks() {
  ...
  if (G.lastMove) { mk(G.lastMove.f, 'lastfrom'); mk(G.lastMove.t, 'lastto'); }
...
/* 떠난 자리 → 놓인 자리로 화살표를 그림. 말에 가리지 않게 양끝을 잘라 둠. */
function renderArrow() {
  const svg = $('layer-arrow');
  if (!G.lastMove) { svg.innerHTML = ''; return; }
  const [fr, fc] = dispRC(G.lastMove.f), [tr, tc] = dispRC(G.lastMove.t);
  const x1 = svgX(fc), y1 = svgY(fr), x2 = svgX(tc), y2 = svgY(tr);
  const dx = x2 - x1, dy = y2 - y1;
  const len = Math.hypot(dx, dy) || 1;
  const ux = dx / len, uy = dy / len;
  const cut1 = 40, cut2 = 66;                      /* 말 지름의 절반쯤 비워 둠 */
  if (len <= cut1 + cut2 + 12) { svg.innerHTML = ''; return; }   /* 한 칸 이동이면 화살표 생략 */
  const ax1 = x1 + ux * cut1, ay1 = y1 + uy * cut1;
  const ax2 = x2 - ux * cut2, ay2 = y2 - uy * cut2;
  svg.innerHTML =
    \`<defs><marker id="arrowhead" markerWidth="4" markerHeight="4" refX="2.6" refY="2" orient="auto">\` +
    \`<polygon points="0 0, 4 2, 0 4" fill="#ff9d2e"/></marker></defs>\` +
    \`<line x1="\${ax1}" y1="\${ay1}" x2="\${ax2}" y2="\${ay2}" stroke="rgba(60,30,0,.5)" stroke-width="18" stroke-linecap="round"/>\` +
    \`<line x1="\${ax1}" y1="\${ay1}" x2="\${ax2}" y2="\${ay2}" stroke="#ff9d2e" stroke-width="11" stroke-linecap="round" marker-end="url(#arrowhead)"/>\`;
}
...
function applyMove(m, tag) {
  const f = (m / 90) | 0, t = m % 90;
  const mover = bd[f], cap = bd[t];
  const label = nameOf(mover) + ' ' + coordLabel(f) + '→' + coordLabel(t) + (cap ? ' ×' + nameOf(cap) : '') + (tag || '');
...
function afterMove() {
  ...
  /* 같은 자리를 오가는 데에는 아무 벌칙도, 판을 끊는 일도 없다.
     한 점도 잡히지 않은 채 아주 오래 끌 때에만 점수로 가린다. */
...
function aiTurn() {
  ...
  requestAnimationFrame(() => setTimeout(async () => {
    ...
    /* 장부에 적힌 적이 있는 수면 기보에 표시 */
    const known = (G.mode === 'ai' && S.learn && bookDelta(posKey(), res.move) !== 0) ? ' 📖' : '';
    applyMove(res.move, known);
  }, 30));
}
...
/* ============================================================================
   6-1. 대국에서 배우기 (학습 장부)
   ----------------------------------------------------------------------------
   대국이 끝나면, 그 판에서 컴퓨터가 두었던 수를 자리마다 승·무·패로 적어 둔다.
   다음에 같은 자리를 만나면 그 성적을 보고 수를 고른다.
   즉 "전에 이 자리에서 이렇게 두었다가 졌으니 다른 수를 보자"를 익히는 것이다.
   신경망 같은 것이 아니라 제 대국 기록으로 만드는 정석책(오프닝 북)이다.
   기록은 이 기기의 브라우저 안에만 남고 어디로도 보내지 않는다.
   ============================================================================ */
const BOOKKEY = 'daram-janggi-book-v1';
const BOOK_PLIES = 30;        /* 초반 서른 수까지만 적어 둠 */
const BOOK_MAX = 4000;        /* 저장소가 넘치지 않게 자리 수 상한 */
const BOOK_MIN_N = 2;         /* 두 판 이상 겪은 자리부터 반영 */

let BOOK = null;              /* { games, wins, draws, losses, pos: { 자리: { 수: [승,무,패] } } } */
function loadBook() {
  if (BOOK) return BOOK;
  try {
  ...
  if (!BOOK || !BOOK.pos) BOOK = { games: 0, wins: 0, draws: 0, losses: 0, pos: {} };
  return BOOK;
}
function saveBook() {
  try { localStorage.setItem(BOOKKEY, JSON.stringify(BOOK)); } catch (e) { /* 저장 못 해도 대국은 계속됨 */ }
}
function bookSize() { return Object.keys(loadBook().pos).length; }

/* 자리마다 적힌 성적을 점수 가감으로 바꿈.
   이겼던 수는 올리고 졌던 수는 내린다. 겪은 판이 적으면 조금만 움직인다. */
function bookDelta(key, mv) {
  const rec = loadBook().pos[key];
  if (!rec) return 0;
  const e = rec[mv];
  if (!e) return 0;
  const [w, d, l] = e, n = w + d + l;
  if (n < BOOK_MIN_N) return 0;
  return Math.round(40 * (w - l) / (n + 1));      /* 최대 ±40 (점수제 4점어치) */
}

/* 대국이 끝나면 그 판을 장부에 적음. res: 'w' | 'd' | 'l' (컴퓨터 기준) */
function learnFromGame(aiSide, res) {
  if (!S.learn || G.mode !== 'ai') return;
  const bk = loadBook();
  bk.games++;
  if (res === 'w') bk.wins++; else if (res === 'd') bk.draws++; else bk.losses++;
  const slot = res === 'w' ? 0 : (res === 'd' ? 1 : 2);
  let added = 0;
  for (let i = 0; i < G.stack.length && i < BOOK_PLIES; i++) {
    const r = G.stack[i];
    if (r.turn !== aiSide || r.m === -1) continue;
    const key = G.posKeys[i];                       /* 그 수를 두기 직전의 자리 */
    if (!key) continue;
    let rec = bk.pos[key];
    if (!rec) {
      if (bookSize() >= BOOK_MAX) continue;         /* 상한을 넘으면 새 자리는 적지 않음 */
      rec = bk.pos[key] = {}; added++;
    }
    const e = rec[r.m] || (rec[r.m] = [0, 0, 0]);
    e[slot]++;
  }
  void added;
  saveBook();
...
/* 탐색이 쓸 보정 함수를 대국 시작 때 걸어 둔다 */
function hookBook() {
  bookBonus = (G.mode === 'ai' && S.learn)
    ? (mv => bookDelta(posKey(), mv))
    : null;
}
...
const PZ_HOW = { 1: '한 수로 끝내기', 2: '두 수 만에 몰아넣기', 3: '세 수 만에 몰아넣기',
                 4: '네 수 만에 몰아넣기', 5: '다섯 수 만에 몰아넣기' };
...
/* 수비의 응수를 고름 — 가장 오래 버티는 수.
   덤으로 "사람이 제대로 두었는지"까지 여기서 가린다. 응수 가운데 하나라도
   그 뒤에 외통이 서지 않는 것이 있으면, 사람이 둔 수가 틀린 것이므로 -1 을 돌려준다.
   (사람이 둔 직후에는 수비 차례이므로, 외통 여부도 수비의 응수마다 따져야 맞다) */
function stubbornReply(rep, k) {
  ...
  for (const r of rep) {
    ...
    unmk(r, cap);
    if (j === 0) return -1;                  /* 이 응수 뒤에는 외통이 없음 */
...
function afterPuzzleMove() {
  ...
  if (remain <= 0) return puzzleFailed();
  if (rep.length === 0) {                     /* 장군이 아닌데 둘 수가 없으면 한수쉼 */
    G.puzzle.left = remain;
    return setTimeout(() => doPass(true), 320);
  }
  G.thinking = true; updateStatus(); updatePanel();
  requestAnimationFrame(() => setTimeout(() => {
    const reply = stubbornReply(rep, remain);
    G.thinking = false;
    if (reply < 0) return puzzleFailed();     /* 응수 하나라도 빠져나가면 잘못 둔 수 */
    G.puzzle.left = remain;
    applyMove(reply);
  }, 320));
}
...
function endGame(reason, winner, detail) {
  ...
  if (G.mode === 'ai') {
    ...
    saveStat(G.level, r);
    /* 컴퓨터 기준으로 뒤집어 장부에 적음 */
    learnFromGame(other(G.humanSide), r === 'w' ? 'l' : (r === 'l' ? 'w' : 'd'));
  }
  setModalButtons();
...
$('btn-hint').addEventListener('click', () => {
  ...
  requestAnimationFrame(() => setTimeout(async () => {
    ...
    try {
      if (G.mode === 'puzzle') {
        mv = firstMatingMove(CHO, G.puzzle.left);
      } else {
...
rootBan = m => (G.mode !== 'puzzle' && !G.over && isPointlessRepeat(m, G.turn, G.stack, G.posKeys));
});
...
function renderBookSummary() {
  ...
  const n = bookSize();
  const tiles = [
    ['배운 대국', bk.games], ['익힌 자리', n],
    ['컴퓨터 승', bk.wins], ['무', bk.draws], ['컴퓨터 패', bk.losses]
  ];
  $('book-summary').innerHTML =
    '<div id="stats-summary" style="margin:12px 0 4px;">' +
    tiles.map(([k, v]) => \`<div class="stat-tile"><div class="sv">\${v}</div><div class="sl">\${k}</div></div>\`).join('') +
    '</div>' +
    (bk.games === 0
      ? '<p class="hint-line" style="text-align:center;">아직 배운 대국이 없습니다. 컴퓨터와 한 판 두고 나면 쌓이기 시작합니다.</p>'
      : (n === 0 ? '<p class="hint-line" style="text-align:center;">적어 둔 자리가 없습니다.</p>' : ''));
}
...
$('btn-reset-book').addEventListener('click', () => {
  askConfirm('배운 것을 지우시겠습니까?', '컴퓨터가 지금까지 익힌 자리 기록이 모두 사라집니다.', () => {
    try { localStorage.removeItem(BOOKKEY); } catch (e) {}
    BOOK = null; renderBookSummary(); toast('배운 것을 지웠습니다.');
  });
});
...
$('opt-learn').checked = S.learn !== false;
...
$('opt-learn').addEventListener('change', e => { S.learn = e.target.checked; saveSettings(); hookBook(); });
